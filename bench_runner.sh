...
if [ $1 == "-h" ] || [ $1 == "--help" ] ; then
    echo "USAGE: NUM_DOMAINS=<num_domains> RUNS=<runs> ./bench_runner.sh <bench_name> <lo> <hi> <step_fn> <percent_change_in_change_prop>"
    ...
    echo "- If \$NUM_DOMAINS is not specified, it'll take the default value(4)"
    exit 1
fi
